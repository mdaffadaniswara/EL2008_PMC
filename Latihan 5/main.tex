...
\While{$z \neq S$}{
u := a vertex not in S with \textit{L(u)} minimal\\
\textit{S} := \textit{S} U \{u\}\\
\For{\textit{all vertices v not in S}}{
\If{$L(u) + w(u,v) < L(v)$}{
    $L(v) := L(u) + w(u,v)$\\
    \{this adds adds a vertex to S with minimal label
...
\If{!is\_final[i] \textbf{and} !is\_found}{
    $idx_min \gets i$\;
    $val_minimum \gets jarak_f[i]$\;
    $is_found \gets true$\;}
\If{is\_final \textbf{and} !is\_found[i] \textbf{and} (jarak\_f[i] < val\_minimum)}{
    $idx_min \gets i$\;
    $val_minimum \gets jarak_f[i]$\;
}
}
\eIf{is\_found}{
    \textbf{return} \textif{(idx\_min)}
}{
    \textbf{return} \textif{(int\_max)}
}
\end{algorithm}
...
\begin{algorithm}[hbt!]
\caption{Program Utama Pencarian Rute Antara
Dua Tanaman - Memasukkan Graph}\label{alg:two}
...
\begin{algorithm}[hbt!]
\caption{Program Utama Pencarian Rute Antara
Dua Tanaman: Pencarian Jarak dengan Algoritma Dijkstra}\label{alg:two}
\KwResult{Menyimpan graph dalam sebuah matriks n x n}
\textbf{Input}(idx\_a) \\
...
$jarak\_dilalui[idx\_a] \gets idx\_a$\;
$idx\_lalui[idx\_a] \gets idx\_lalui[idx\_a]+1$\;
$idx\_now[idx\_a] \gets idx\_a$\;
\While{idx\_now != int_max}{
    $is\_final[idx\_now] \gets TRUE$\; 
    \For{i = 0 $\textbf{to } n\_tanaman-1$}{
        \If{(!is\_final[i]) \textbf{and}
        graph[idx\_now][i]! = int\_max \textbf{and}
        (jarak_f[idx\_now] +
        graph[idx\_now][i]) > jarak\_f[i]}{
            $jarak\_f[i] \gets jarak_f[idx\_now] + graph[idx\_now][i]$\;
            $idx\_lalui[i] \gets idx\_lalui[idx\_now] + 1$\;
        }
        \For{j = 0 $\textbf{to } idx\_lalui[i]-1$}{
            \eIf{j=idx\_dilalui[i]-1}{
                $list\_lalui[i][j] \gets i$\;                
            }{
                $list\_lalui[i][j] \gets list\_lalui[idx\_now][j]$\;                
            }
        }
    }
    $idx\_now \gets idx\_process (n\_tanaman,
    jarak\_f, is\_final)$\;   
}
\end{algorithm}
